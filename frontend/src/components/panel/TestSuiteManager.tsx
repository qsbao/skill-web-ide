  ...
  if (!activeSkillId) {
    return (
      <div className="h-full flex flex-col items-center justify-center bg-surface-base">
        <ClipboardList className="w-6 h-6 text-slate-600 mb-2" />
        <span className="text-slate-500 text-xs">Select a skill to manage test cases</span>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto">
        {cases.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8">
            <ClipboardList className="w-5 h-5 text-slate-600 mb-2" />
            <span className="text-xs text-slate-500">No test cases yet.</span>
          </div>
        ) : (
          cases.map((tc) => (
            <div key={tc.id} className="px-3 py-2.5 border-b border-border-subtle/30 hover:bg-surface-overlay/30 transition-colors">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-200">{tc.name}</span>
                </div>
                <div className="flex gap-0.5">
                  <button
                    onClick={() => handleEdit(tc)}
                    className="btn-ghost !px-1.5 !py-1 text-slate-500 hover:text-accent"
                  >
                    <Edit2 className="w-3 h-3" />
                  </button>
                  <button
                    onClick={() => handleDelete(tc.id)}
                    className="btn-ghost !px-1.5 !py-1 text-slate-500 hover:text-red-400"
                  >
              ...
              {tc.input && (
                <pre className="text-[10px] text-slate-500 mt-1 truncate font-mono">{tc.input}</pre>
              )}
            </div>
          ))
        )}
